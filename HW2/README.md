...
5)  Level 3 물체의 크기를
    \(scale = 1.0 + 0.25*sin(3.0*radian\left( t \right))\)으로 설정하여,
...
6)  각 Level의 물체 이동 방식은 아래와 같습니다. (과제 구현을 명세서 수정 이전에 시작해 전체 계층은 Level 5까지
    ...
    Level 2: Level 1을 기준으로 \(x = 80.0*\cos\left( t \right)\),
    \(y = 80.0*\sin\left( 3.0*\ t \right)\)의 파형 운동  
    Level 3: Level 2를 기준으로 Level 2와의 offset이
    \(x = 200.0 + 40.0*\sin\left( 2.0*t \right)\)로 변하면서 회전하는 운동.  
    Level 4: Level 1부터 Level 3까지 모든 부모의 rotate에 대한 영향을 상쇄시켜 항상 crane이
    아래를 볼 수 있게 하는 운동  
    Level 5: \(angle = 40.0 + 20.0*sin(t)\)로 Level 4의 중심 좌표 기준으로
